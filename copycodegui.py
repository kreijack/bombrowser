...
class CopyCode(QDialog):
    ...
    def _init_gui(self):
        ...
        grid.addWidget(QLabel("Old code:"), 10, 0)
        self._l_old_code = QLabel(self._old_code)
        grid.addWidget(self._l_old_code, 10, 1)

        grid.addWidget(QLabel("Old revision:"), 10, 2)
        self._l_old_revision = QLabel("%d"%(self._old_iter))
        grid.addWidget(self._l_old_revision, 10, 3)

        grid.addWidget(QLabel("Old description:"), 10, 4)
        self._l_old_descr = QLabel(self._old_descr)
        grid.addWidget(self._l_old_descr, 10, 5)

        grid.addWidget(QLabel("New code:"), 11, 0)
        self._l_new_code = QLineEdit(self._old_code)
        grid.addWidget(self._l_new_code, 11, 1)

        grid.addWidget(QLabel("New revision:"), 11, 2)
        self._l_new_iter = QLineEdit()
        self._l_new_iter.setReadOnly(True)
        grid.addWidget(self._l_new_iter, 11, 3)

        grid.addWidget(QLabel("New description:"), 11, 4)
        self._l_new_descr = QLineEdit(self._old_descr)
        grid.addWidget(self._l_new_descr, 11, 5)
    ...
    def _do(self):
        ...
        try:
            if self._cb_copy_rev.checkState() == Qt.CheckState.Checked:
                    d.copy_code(self._l_new_code.text(),
                        self._rid,
